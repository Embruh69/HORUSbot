...
def build_encounter_embed(enemies: list[NpcEnemy], title: str = "⚔️ Encounter Roster") -> discord.Embed:
    ...
    if alive:
        ...
        for e in alive:
            ...
            heat_str = f"  🔥{s.current_heat}/{s.heatcap}" if s.current_heat > 0 else ""
            burn_str = f"  🔥Burn {s.burn}" if s.burn else ""
            lines.append(
                f"{active_marker} **{e.display_name}**  "
                f"[{e.role.title()} T{e.tier}]\n"
                f"  ❤️ `{hp_bar}` {s.current_hp}/{s.hp}{heat_str}{burn_str}"
            )
...
def build_add_result_embed(added: list[NpcEnemy]) -> discord.Embed:
    ...
    for e in added:
        ...
        lines.append(
            ...
            f"❤️ {s.hp} HP  🛡️ {s.armor} Armor  ⚡{s.speed} Spd"
        )
    embed.description = "\n".join(lines)
    embed.set_footer(text="!npc list  ·  !npc show <name>  ·  !npc activate <name>")
    return embed
