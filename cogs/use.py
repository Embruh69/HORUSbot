"""
Use cog — !use and !attack commands for weapons and systems.
...
def build_weapon_use_embed(
    ...
    # Tags
    tag_ids = weapon.tag_ids
    if tag_ids:
        embed.add_field(name="🏷️ Tags", value=", ".join(tag_ids), inline=True)
...
class UseCog(commands.Cog, name="Use"):
    """Commands for using weapons and systems."""
    ...
    async def _resolve_item(
        ...
        """
        Look up an item by fuzzy name. Handles disambiguation automatically.
        Returns the chosen item, or None if nothing found.
        """
        ...
        if not query:
            ...
            embed = discord.Embed(
                title="🗂️ Available Weapons & Systems",
                description="\n".join(lines) or "None equipped.",
                color=DEFAULT_COLOR,
            )
            embed.set_footer(text="Usage: !use <name>  ·  !use <name> acc 2  ·  !use <name> diff 1")
        ...
        if not matches:
            await ctx.reply(
                ...
                "Run `!use` with no arguments to see all available items.",
        ...
        # Multiple matches — check if they all have the same name
        names = {m.name.lower() for m in matches}
        if len(names) == 1:
            # Same name, different types — let user pick
            pass

        # Disambiguation
        view = DisambiguateView(matches, ctx, accuracy, difficulty)
        embed = discord.Embed(
            title="🤔 Multiple matches — which did you mean?",
            description="\n".join(
        ...
        if view.chosen is None:
            await msg.edit(content="⏱️ Selection timed out.", embed=None, view=None)
            return None
    ...
    async def use_item(self, ctx: commands.Context, *args: str):
        """
        Use a weapon or system by name (fuzzy, case-insensitive).

        Usage:
            !use torch
            !use beckoner
            !use lightning generator acc 2
            !use lucifer diff 1
            !use                    ← list all items

        Accuracy / Difficulty flags (any of these work):
            acc 2  |  a2  |  accuracy 2
            diff 1 |  d1  |  difficulty 1
        """
        ...
        async with ctx.typing():
            if isinstance(item, Weapon):
                await self._use_weapon(ctx, char, item, accuracy, difficulty)
            else:
                await self._use_system(ctx, char, item, accuracy, difficulty)

    # ── !attack ───────────────────────────────────────────────────────────────

    @commands.command(name="attack", aliases=["atk", "a"])
    async def attack(self, ctx: commands.Context, *args: str):
        """
        Attack with a weapon — alias for !use that makes intent explicit.
        Only matches weapons (not systems).

        Usage:
            !attack torch
            !attack heavy melee acc 1
        """
        accuracy, difficulty = self._parse_acc_diff(args)
        item_name = self._parse_item_name(args)
        ...
        if not item_name:
            lines = [f"⚔️ **{w.name}**  ({w.damage_str})" for w in mech.weapons]
            embed = discord.Embed(
                title="⚔️ Available Weapons",
                description="\n".join(lines) or "No weapons equipped.",
                color=DEFAULT_COLOR,
            )
            await ctx.reply(embed=embed, mention_author=False)
            return

        matches = _fuzzy_match(item_name, mech.weapons)

        if not matches:
            await ctx.reply(
                f"❌ No weapon matching **\"{item_name}\"**.\n"
                "Run `!attack` with no arguments to list weapons.",
                mention_author=False,
            )
            return
        ...
        if len(matches) > 1:
            view = DisambiguateView(matches, ctx, accuracy, difficulty)
            embed = discord.Embed(
                title="🤔 Multiple weapons match — which did you mean?",
                description="\n".join(f"⚔️ **{m.name}**" for m in matches[:5]),
                color=0xFFC107,
            )
            msg = await ctx.reply(embed=embed, view=view, mention_author=False)
            await view.wait()
        ...
        async with ctx.typing():
            await self._use_weapon(ctx, char, weapon, accuracy, difficulty)

    # ── !activate ─────────────────────────────────────────────────────────────

    @commands.command(name="activate", aliases=["act", "sys"])
    async def activate_system(self, ctx: commands.Context, *args: str):
        """
        Activate a system — alias for !use that only searches systems.

        Usage:
            !activate beckoner
            !activate lightning generator
            !activate lucifer diff 1
        """
        accuracy, difficulty = self._parse_acc_diff(args)
        item_name = self._parse_item_name(args)
        ...
        if not item_name:
            lines = [f"🔧 **{s.name}**" + (f"  ({s.type})" if s.type else "") for s in mech.systems]
            embed = discord.Embed(
                title="🔧 Available Systems",
                description="\n".join(lines) or "No systems equipped.",
                color=DEFAULT_COLOR,
            )
            await ctx.reply(embed=embed, mention_author=False)
            return

        matches = _fuzzy_match(item_name, mech.systems)

        if not matches:
            await ctx.reply(
                f"❌ No system matching **\"{item_name}\"**.\n"
                "Run `!activate` with no arguments to list systems.",
                mention_author=False,
            )
            return
        ...
        if len(matches) > 1:
            view = DisambiguateView(matches, ctx, accuracy, difficulty)
            embed = discord.Embed(
                title="🤔 Multiple systems match — which did you mean?",
        ...
        async with ctx.typing():
            await self._use_system(ctx, char, system, accuracy, difficulty)
    ...
    async def _use_weapon(
        ...
        # Sum up totals for the view buttons
        total_dmg = sum(r.total for _, r in damage_rolls if not d.get("target") == "self" for d, r in [(d, r)])
        # Recompute cleanly
        ...
        embed = build_weapon_use_embed(
            char, weapon, attack, damage_rolls, self_heat_rolls, accuracy, difficulty
        )
